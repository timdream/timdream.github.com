...
CVCipherUI.prototype = {
  ...
  decipherLoadEnd: function(msg) {
    ...
    switch (msg.type) {
      case "error":
        alert(
          "Sorry, there were some problem decrypting the information. " +
  ...
  load: function(password) {
    ...
    if (this.useWebCrypto) {
      worker = this.worker = new Worker(
        "/assets/webcrypto-decipher.min.js?_=%hash-webcrypto-decipher.min.js%"
      );
      this.decipherName = "Web Crypto Decipher";
    } else {
      worker = this.worker = new Worker(
        "/assets/asmcrypto-decipher.min.js?_=%hash-asmcrypto-decipher.min.js%"
      );
